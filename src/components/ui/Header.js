...
export default function Header(props) {
  ...
  useEffect(() => {
    ...
    //   setValue(1)
    //   setSelectedIndex(1)
    } else if (window.location.pathname === '/join' && value !== 1) {
      setValue(1)
      setSelectedIndex(1)
    } else if (window.location.pathname === '/as-dev' && value !== 1) {
      setValue(1)
      setSelectedIndex(1)
    } else if (window.location.pathname === '/as-client' && value !== 1) {
      setValue(1)
      setSelectedIndex(1)
    ...
    <React.Fragment>
      <SwipeableDrawer disableBackdropTransition={!iOS}
        ...
        <List disablePadding>
          ...
          <ListItem divider button disableRipple onClick={()=> {setOpenDrawer(false); setValue(1)}} selected={value === 1} component={Link} to="/join" >
            <ListItemText disableTypography className={value === 1 ? [classes.drawerItemSelected, classes.drawerItem] : classes.drawerItem }>Join</ListItemText>
          </ListItem>
          <ListItem divider button disableRipple onClick={()=> {setOpenDrawer(false); setValue(2)}} selected={value === 2} component={Link} to="/dashboard">
            <ListItemText disableTypography className={value === 2 ? [classes.drawerItemSelected, classes.drawerItem] : classes.drawerItem }>Dashboard</ListItemText>
          </ListItem>
    ...
    <React.Fragment>
      ...
      <Menu id="simple-menu" anchorEl={anchorEl} open={openMenu} onClose={handleClose} MenuListProps={{ onMouseLeave: handleClose }}
        ...
        {menuOptions.map((option, i) => (
          <MenuItem key={option.name} component={Link} to={option.link} classes={{ root: classes.menuItem }} onClick={(e) => { handleMenuItemClick(e, i); setValue(1); handleClose() }} selected={i === selectedIndex}>
            {option.name}
          </MenuItem>
        ))}
      </Menu>
